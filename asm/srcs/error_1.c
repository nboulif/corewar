...
{
	return (ft_printf(g_err_msgs[ERROR_MALLOC], str, nb_line));
}

int		err_param_type(t_prog *p, char *str, int indice)
{
	return (ft_printf(g_err_msgs[ERROR_PARAM_TYPE], p->nb_line, str, indice));
}

int		err_missing_quotes(t_prog *p, char *error_type, int start)
{
	if (start)
		return (ft_printf(g_err_msgs[ERROR_MISSING_QUOTE],
